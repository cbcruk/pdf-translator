...
| — scanned path | `pdf-cli structure` (Swift) — or [structure-tesseract.ts](src/pipeline/structure-tesseract.ts) via `--ocr tesseract` — + [structure-blocks.ts](src/pipeline/structure-blocks.ts) | `RecognizeDocumentsRequest` paragraphs/titles/tables/lists → blocks directly |
